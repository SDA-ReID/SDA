...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 num_features=0, norm=False, dropout=0, num_classes=0, process_group=None):
        ...
        # , norm_layer=norm_layer, process_group=process_group)
        ...
        if not self.cut_at_pooling:
            ...
            if self.has_embedding:
                ...
                # self.feat_bn = nn.SyncBatchNorm(self.num_features, process_group=process_group)
    ...
    def forward(self, x):
        ...
        if self.has_embedding:
            bn_x = self.feat_bn(self.feat(x)[:,:,None]).view(x.size(0),-1)
        else:
            bn_x = self.feat_bn(x[:,:,None]).view(x.size(0),-1)
